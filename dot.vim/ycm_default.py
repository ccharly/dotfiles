...
def MakeCompilationDatabase(path):
  if os.path.exists(path):
    return ycm_core.CompilationDatabase(path)
  else:
...
class Ycm:
  def __init__(self, opt):
    ...
    self.flags = []
    self.database = None
    if 'compilation_database' in opt:
      self.database = MakeCompilationDatabase(opt['compilation_database'])
    if 'flags' in opt:
  ...
  def FlagsForFile(self, filename, **kwargs):
    ...
    return {
      'flags': flags + final_flags + self.flags,
      'do_cache': True
    }
